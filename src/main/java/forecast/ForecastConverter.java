...
public class ForecastConverter {
	...
	private String createNav(String page, String office) {
		...
		page = MarkupUtil.addURL(page, "?office=ARX",
				MarkupUtil.addStartDiv("", null, (office.equals("ARX") ? "current-page" : null)) + "la crosse"
						+ MarkupUtil.addEndDiv(""));
		page = MarkupUtil.addURL(page, "?office=DLH",
				MarkupUtil.addStartDiv("", null, (office.equals("DLH") ? "current-page" : null)) + "duluth"
						+ MarkupUtil.addEndDiv(""));
		page = MarkupUtil.addURL(page, "?office=PSR",
				MarkupUtil.addStartDiv("", null, (office.equals("PSR") ? "current-page" : null)) + "phoenix"
						+ MarkupUtil.addEndDiv(""));
		page = MarkupUtil.addEndDiv(page);
	...
	private String createHeading(String page, String[] body, int sectionStart, int sectionEnd) {
		...
		if (sectionEnd - sectionStart == 2) {
			if (body[sectionStart + 1].length() > 0
					&& ".,;:?!".indexOf(body[sectionStart + 1].charAt(body[sectionStart + 1].length() - 1)) == -1) {
	...
	private String createFooter(String page) {
		...
		String footer = "<div id=\"footercontainer\">\n" + "<div id=\"footer\">\n" + "<p>"
				+ footers[r.nextInt(footers.length)] + "</p>\n" + "</div>\n" + "</div>\n";
